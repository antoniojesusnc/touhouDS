...
void CMovement::initFrames(CXMLParser::TXML* movementData) {
	...
	_hitBoxPerFrame = (Rect**)malloc(sizeof(Rect*)*_numFrames);
	_priorityPerFrame = (u8*)malloc(sizeof(u8)*_numFrames); 
	...
	for(vu8 i = 0; i < _numFrames; ++i){
		...
		_damagePerFrame[i] = 0;
		_priorityPerFrame[0] = 0;
		_durationPerFrame[i] = 0;
		_hitBoxPerFrame[i] = NULL;
	}
	
	
	char* auxTagFrame;
	for(vu8 i = 0; i < _numFrames; ++i){
		for(vu8 k = 0; k < movementData->childs[i]->numChilds; ++k){
			//printf("%d", movementData->childs[i]->numChilds);
			...
			}else if(strcmp(auxTagFrame, "priority") == 0){
				//printf("\n priotiry");
				_priorityPerFrame[i] = atoi(movementData->childs[i]->childs[k]->value);
				// hit box
			}else if(strcmp(auxTagFrame, "hitX") == 0){
					//printf("\n asdf");
					if(_hitBoxPerFrame[i] == NULL) _hitBoxPerFrame[i] = new Rect();
					_hitBoxPerFrame[i]->setX(atoi(movementData->childs[i]->childs[k]->value));
				}else if(strcmp(auxTagFrame, "hitY") == 0){
					if(_hitBoxPerFrame[i] == NULL) _hitBoxPerFrame[i] = new Rect();
					_hitBoxPerFrame[i]->setY(atoi(movementData->childs[i]->childs[k]->value));
				}else if(strcmp(auxTagFrame, "hitWidth") == 0){
					if(_hitBoxPerFrame[i] == NULL) _hitBoxPerFrame[i] = new Rect();
					_hitBoxPerFrame[i]->setWidth(atoi(movementData->childs[i]->childs[k]->value));
				}else if(strcmp(auxTagFrame, "hitHeight") == 0){
					if(_hitBoxPerFrame[i] == NULL) _hitBoxPerFrame[i] = new Rect();
					_hitBoxPerFrame[i]->setHeight(atoi(movementData->childs[i]->childs[k]->value));
				}
		}
	} // end for frames
} // initFrames
